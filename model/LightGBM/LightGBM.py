...
def optimize_lightgbm_hyperparameters(X_train, y_train, X_val, y_val, n_trials=50):
...
def train_lightgbm(X_train, y_train, X_val, y_val, use_tuning=True, n_trials=50):
    """LightGBM 모델 학습 (이진 분류)"""
    # 하이퍼파라미터 튜닝
    if use_tuning:
        best_params = optimize_lightgbm_hyperparameters(X_train, y_train, X_val, y_val, n_trials)
        # 기본 파라미터와 최적 파라미터 병합
        params = {
            'objective': 'binary',
            'metric': 'binary_logloss',
...
def main():
    ...
    # 5. 모델 학습
    print("\n[5단계] LightGBM 모델 학습")
    model, best_params = train_lightgbm(X_train_split, y_train_split, X_val, y_val, use_tuning=True, n_trials=50)
