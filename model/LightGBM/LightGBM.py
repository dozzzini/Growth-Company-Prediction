...
def load_data(include_patent_features=True, use_patent_only=False, use_normalized_financial=False, use_train_dataset=False):
    """데이터 로드
    
    Args:
        include_patent_features: 특허 피처 포함 여부 (use_train_dataset=True일 때는 무시됨)
        use_patent_only: True이면 특허 피처만 사용 (기존 재무 피처 제외, use_train_dataset=True일 때는 무시됨)
        use_normalized_financial: True이면 정규화된 재무 피처 사용 (growth_features_normalized.csv, use_train_dataset=True일 때는 무시됨)
        use_train_dataset: True이면 train_dataset.csv 사용 (모든 피처 포함)
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(script_dir))
    
    if use_train_dataset:
        # train_dataset.csv 사용
        print("[train_dataset.csv 사용 모드]")
        train_path = os.path.join(project_root, 'data', 'train_dataset.csv')
        features_df = pd.read_csv(train_path, encoding='utf-8-sig')
        print(f"train_dataset.csv 로드: {features_df.shape}")
    ...
    if use_patent_only:
        # 특허 피처만 사용하는 경우
        print("[특허 피처만 사용 모드]")
        patent_path = os.path.join(project_root, 'data', 'normalized_patent_features.csv')
        try:
            features_df = pd.read_csv(patent_path, encoding='utf-8-sig')
            print(f"특허 피처 데이터 로드: {features_df.shape}")
            print(f"  특허 피처 컬럼 수: {len(features_df.columns) - 4}개 (기업명, 연도, company_norm, target_year 제외)")
        except FileNotFoundError:
            raise FileNotFoundError(f"특허 피처 파일을 찾을 수 없습니다: {patent_path}")
        except Exception as e:
            raise Exception(f"특허 피처 로드 중 오류 발생: {e}")
    else:
        # 재무 피처 로드
        if use_normalized_financial:
            # 정규화된 재무 피처 사용
            print("[정규화된 재무 피처 사용 모드]")
            ...
            features_df = pd.read_csv(features_path, encoding='utf-8-sig')
            print(f"재무 피처 데이터 로드: {features_df.shape}")
        
        # 특허 피처 데이터 로드 및 병합 (include_patent_features가 True인 경우만)
        if include_patent_features:
            patent_path = os.path.join(project_root, 'data', 'normalized_patent_features.csv')
            try:
                patent_df = pd.read_csv(patent_path, encoding='utf-8-sig')
                print(f"특허 피처 데이터 로드: {patent_df.shape}")
                
                # 특허 피처에서 사용할 컬럼 선택 (기업명, 연도, 특허 관련 피처만)
                # company_norm, target_year는 제외하고 특허 피처만 선택
                patent_feature_cols = [col for col in patent_df.columns 
                                     if col not in ['company_norm', '기업명', 'target_year', '연도']]
                
                # 기업명과 연도 기준으로 병합
                patent_features = patent_df[['기업명', '연도'] + patent_feature_cols].copy()
                
                # 병합 전 데이터 확인
                print(f"  기존 피처 데이터: {features_df.shape}")
                print(f"  특허 피처 데이터: {patent_features.shape}")
                print(f"  특허 피처 컬럼 수: {len(patent_feature_cols)}개")
                
                # 기업명과 연도 기준으로 left join (기존 피처 기준)
                ...
                print(f"  병합 후 피처 데이터: {features_df.shape}")
                print(f"  특허 피처 결측치: {features_df[patent_feature_cols].isnull().sum().sum()}개")
                
            except FileNotFoundError:
                print(f"경고: 특허 피처 파일을 찾을 수 없습니다: {patent_path}")
                print("특허 피처 없이 진행합니다.")
            except Exception as e:
                print(f"경고: 특허 피처 로드 중 오류 발생: {e}")
                print("특허 피처 없이 진행합니다.")
        else:
            print("특허 피처는 제외하고 진행합니다.")
    
    return features_df, financial_df


def load_test_dataset(project_root=None):
    """test_dataset.csv 파일 로드"""
    if project_root is None:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(os.path.dirname(script_dir))
    
    test_path = os.path.join(project_root, 'data', 'test_dataset.csv')
    
    if os.path.exists(test_path):
        test_df = pd.read_csv(test_path, encoding='utf-8-sig')
        print(f"test_dataset.csv 로드: {test_df.shape}")
...
def create_target_variable(features_df, financial_df):
    ...
    for idx, row in features_df.iterrows():
        ...
        if pd.notna(rev_t) and pd.notna(rev_t1) and rev_t > 0:
            ...
            # 결측치 발생 원인 기록
            if pd.isna(rev_t):
                reason = f"{feature_year}년 매출액 없음"
            elif pd.isna(rev_t1):
                reason = f"{target_year}년 매출액 없음"
            elif rev_t <= 0:
                reason = f"{feature_year}년 매출액이 0 이하 ({rev_t})"
            else:
                reason = "알 수 없는 이유"
    ...
    # 결측치 발생 케이스 출력
    if missing_cases:
        print(f"\n[타겟 변수 생성] 매출액 데이터 부족으로 인한 결측치: {len(missing_cases)}개")
        missing_df = pd.DataFrame(missing_cases)
        print("결측치 발생 기업:")
        for _, row in missing_df.iterrows():
            print(f"  - {row['기업명']}: {row['reason']} ({row['feature_year']}년 피처 → {row['target_year']}년 타겟)")
...
def prepare_data(df, use_patent_only=False):
    """모델링을 위한 데이터 준비
    
    Args:
        df: 피처 데이터프레임
        use_patent_only: True이면 특허 피처만 사용
    """
    # 기업명, 연도, growth_rate, target_year, target_growth, target_top30은 제외
    # 특허 피처의 경우 결측치가 있을 수 있으므로 포함
    exclude_cols = ['기업명', '연도', 'growth_rate', 'target_year', 'target_growth', 'target_top30', 'company_norm']
    
    if use_patent_only:
        # 특허 피처만 선택
        patent_cols = [col for col in df.columns 
                      if col not in exclude_cols and 
                      ('patent' in col.lower() or col in ['ipc_diversity', 'similarity', 'no_patent_flag'])]
        feature_cols = patent_cols
        print(f"[특허 피처만 사용] 특허 피처 {len(feature_cols)}개 선택")
    else:
        # 모든 피처 사용
    ...
    # 결측치 처리
    if use_patent_only:
        # 특허 피처만 사용하는 경우: 결측치를 0으로 채움
        patent_missing = X.isnull().sum().sum()
        if patent_missing > 0:
            print(f"  특허 피처 결측치 처리: {patent_missing}개 → 0으로 채움")
            X = X.fillna(0)
        mask = ~y.isnull()
    else:
        # 기존 방식: 특허 피처는 0으로 채우고, 다른 피처는 행 제거
        patent_cols = [col for col in feature_cols if 'patent' in col.lower() or col.startswith('patent_')]
        other_cols = [col for col in feature_cols if col not in patent_cols]
        
        # 특허 피처 결측치를 0으로 채움 (특허가 없는 경우)
        if patent_cols:
            patent_missing = X[patent_cols].isnull().sum().sum()
            if patent_missing > 0:
                print(f"  특허 피처 결측치 처리: {patent_missing}개 → 0으로 채움")
                X[patent_cols] = X[patent_cols].fillna(0)
        
        # 다른 피처와 타겟 변수의 결측치가 있는 행 제거
        mask = ~(X[other_cols].isnull().any(axis=1) | y.isnull())
    
    X = X[mask]
    y = y[mask]
    
    print(f"최종 데이터 shape: {X.shape}")
    if not use_patent_only:
        patent_cols = [col for col in feature_cols if 'patent' in col.lower() or col.startswith('patent_')]
        other_cols = [col for col in feature_cols if col not in patent_cols]
        print(f"  피처 수: {len(feature_cols)}개 (기존: {len(other_cols)}개, 특허: {len(patent_cols)}개)")
    else:
        print(f"  피처 수: {len(feature_cols)}개 (특허 피처만)")
...
def save_performance_report(train_metrics, val_metrics, test_metrics, best_params, 
                           feature_importance_df, script_dir, model_name='LightGBM'):
...
def rolling_validation(X, y, features_df_filtered, model_params, years=[2021, 2022, 2023, 2024]):
    ...
    for i, val_year in enumerate(years):
        ...
        # 모델 학습
        train_data = lgb.Dataset(X_train_roll, label=y_train_roll)
        val_data = lgb.Dataset(X_val_roll, label=y_val_roll, reference=train_data)
        ...
        # 평가
        y_pred_proba = model.predict(X_val_roll, num_iteration=model.best_iteration)
...
def main():
    ...
    use_normalized_financial = True  # 정규화된 재무 피처 사용 (use_train_dataset=False일 때만 사용)
    include_patent_features = False 
    features_df, financial_df = load_data(
        include_patent_features=include_patent_features, 
        use_patent_only=use_patent_only,
        use_normalized_financial=use_normalized_financial,
    ...
    # 3. 데이터 준비
    print("\n[3단계] 데이터 준비")
    X, y, feature_cols = prepare_data(features_df, use_patent_only=use_patent_only)
    ...
    # Train: 2019~2023년 피처 (2020~2024년 타겟 예측용)
    train_mask = features_df_filtered['연도'] <= 2023  # 2019-2023년 말 피처
    # Test: 2024년 타겟 (연도별_성장여부_컬럼변환.csv에서 로드)
    # 실제로는 2023년 피처로 2024년을 예측하는 것이므로 test_mask는 따로 계산하지 않음
    ...
    # 하이퍼파라미터 튜닝 여부 설정 (False로 설정하면 기존 best_params.txt 사용)
    use_tuning = False  # True: 하이퍼파라미터 튜닝 실행, False: 기존 파라미터 사용
    ...
    if use_tuning:
        ...
        if os.path.exists(params_path):
            ...
            print(f"기존 하이퍼파라미터 로드: {params_path}")
            print(f"로드된 파라미터: {list(best_params.keys())}")
        else:
            print(f"경고: {params_path} 파일이 없습니다. 기본 파라미터를 사용합니다.")
            best_params = {}
        ...
        with mlflow.start_run(run_name=f"LightGBM_Run_{datetime.now().strftime('%H%M%S')}"):
            model, _ = train_lightgbm(X_train_split, y_train_split, X_val, y_val, use_tuning=False, cv=5)
            ...
            mlflow.log_param("cv_folds", 5)
            mlflow.log_param("n_estimators", 1000)
            mlflow.log_param("use_tuning", False)
    ...
    # 최종 테스트 (시간 홀드아웃): test_dataset.csv 사용
    holdout_results = {}
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(script_dir))
    ...
    # test_dataset.csv 로드
    if use_train_dataset:
        print("\n[최종 테스트] test_dataset.csv 사용 (2023년 피처 → 2024년 타겟)")
        ...
        if len(test_df) > 0 and 'target_growth' in test_df.columns:
            ...
            X_test_df, y_test_df, _ = prepare_data(test_df, use_patent_only=use_patent_only)
            
            if len(X_test_df) > 0:
                # 예측 수행
                y_test_pred_proba = model.predict(X_test_df, num_iteration=model.best_iteration)
        ...
        if len(X_test_2024) > 0:
            ...
            if len(actual_target_df) > 0:
                # 2023년 피처로 예측
                y_test_pred_proba = model.predict(X_test_2024, num_iteration=model.best_iteration)
    ...
    save_performance_report(
        ...
        best_params, feature_importance_df, script_dir, model_name='LightGBM'
    )
