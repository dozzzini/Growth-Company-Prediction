...
def load_data(use_train_dataset=True):
    ...
    Args:
        use_train_dataset: True이면 train_dataset.csv 사용
    """
...
def optimize_randomforest_hyperparameters(X_train, y_train, cv=5):
    """단계별 그리드 탐색을 사용한 RandomForest 하이퍼파라미터 최적화"""
    print("\n[하이퍼파라미터 튜닝 시작]")
    ...
    # 단계 1: 트리 구조 파라미터 탐색
    print("\n[단계 1/4] 트리 구조 파라미터 탐색 (max_depth, min_samples_split, min_samples_leaf)")
    step1_grid = {
        'max_depth': [10, 20, 30],
    ...
    # 단계 2: max_features 탐색
    print("\n[단계 2/4] max_features 탐색")
    step2_grid = {'max_features': ['sqrt', 'log2', None]}
    
    best_score = -np.inf
    step2_params = {}
    param_list = list(ParameterGrid(step2_grid))
    
    with mlflow.start_run(nested=True, run_name="Step2_Max_Features"):
        for idx, params in enumerate(tqdm(param_list, desc="max_features 탐색", unit="조합")):
            model = RandomForestClassifier(**best_params, **params)
            scores = cross_val_score(model, X_train, y_train, cv=cv, scoring='roc_auc', n_jobs=-1)
            mean_score = scores.mean()
        ...
        print(f"  최적 max_features: {step2_params}, CV ROC-AUC: {best_score:.4f}")
    
    best_params.update(step2_params)
    
    # 단계 3: max_samples 탐색
    print("\n[단계 3/4] max_samples 탐색 (bootstrap sampling)")
    step3_grid = {
        'bootstrap': [True],
    ...
    with mlflow.start_run(nested=True, run_name="Step3_Bootstrap_Sampling"):
        ...
        print(f"  최적 샘플링: {step3_params}, CV ROC-AUC: {best_score:.4f}")
    
    best_params.update(step3_params)
    
    # 단계 4: class_weight 탐색
    print("\n[단계 4/4] class_weight 탐색")
    step4_grid = {'class_weight': [None, 'balanced', 'balanced_subsample']}
    
    best_score = -np.inf
    step4_params = {}
    param_list = list(ParameterGrid(step4_grid))
    
    with mlflow.start_run(nested=True, run_name="Step4_Class_Weight"):
        for idx, params in enumerate(tqdm(param_list, desc="class_weight 탐색", unit="조합")):
            model = RandomForestClassifier(**best_params, **params)
            scores = cross_val_score(model, X_train, y_train, cv=cv, scoring='roc_auc', n_jobs=-1)
            mean_score = scores.mean()
        ...
        print(f"  최적 class_weight: {step4_params}, CV ROC-AUC: {best_score:.4f}")
    
    best_params.update(step4_params)
    
    # 최종 결과 출력 및 MLflow 로깅
    print(f"\n[최종 결과] 최적 하이퍼파라미터:")
    final_params = {k: v for k, v in best_params.items() if k not in ['random_state', 'n_jobs', 'n_estimators']}
...
def train_randomforest(X_train, y_train, X_val, y_val, use_tuning=True, cv=5):
    """RandomForest 모델 학습 (이진 분류)"""
    # 하이퍼파라미터 튜닝
    if use_tuning:
        ...
        best_params = optimize_randomforest_hyperparameters(X_train_val, y_train_val, cv=cv)
...
def plot_feature_importance(model, feature_cols, top_n=15):
    ...
    plt.title(f'Top {top_n} Feature Importance (RandomForest)')
...
def save_performance_report(train_metrics, val_metrics, test_metrics, best_params, 
                           feature_importance_df, script_dir, model_name='RandomForest'):
...
def rolling_validation(X, y, features_df_filtered, model_params, years=[2021, 2022, 2023, 2024]):
    ...
    for i, val_year in enumerate(years):
        ...
        # 모델 학습
        model = RandomForestClassifier(**model_params, verbose=0)
        model.fit(X_train_roll, y_train_roll)
        
        # 평가
        y_pred_proba = model.predict_proba(X_val_roll)[:, 1]
...
def plot_predictions(y_true, y_pred, y_pred_proba, split_name=''):
    ...
    axes[0, 0].set_title(f'{split_name} - Confusion Matrix (RandomForest)')
    ...
    axes[0, 1].set_title(f'{split_name} - ROC Curve (RandomForest)')
    axes[0, 1].legend()
    ...
    axes[1, 0].set_title(f'{split_name} - Probability Distribution (RandomForest)')
    axes[1, 0].legend()
    ...
    axes[1, 1].set_title(f'{split_name} - Precision-Recall Curve (RandomForest)')
...
def main():
    ...
    print("\n[5단계] RandomForest 모델 학습")
    
    # 하이퍼파라미터 튜닝 여부 설정 (False로 설정하면 기존 best_params.txt 사용)
    use_tuning = False  # True: 하이퍼파라미터 튜닝 실행, False: 기존 파라미터 사용
    
    if use_tuning:
        experiment_name = f"RandomForest_Growth_Prediction_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        mlflow.set_experiment(experiment_name)
        
        with mlflow.start_run(run_name=f"RandomForest_Run_{datetime.now().strftime('%H%M%S')}"):
            model, best_params = train_randomforest(X_train_split, y_train_split, X_val, y_val, use_tuning=True, cv=5)
        ...
        with mlflow.start_run(run_name=f"RandomForest_Run_{datetime.now().strftime('%H%M%S')}"):
            model, _ = train_randomforest(X_train_split, y_train_split, X_val, y_val, use_tuning=False, cv=5)
    ...
    if use_train_dataset:
        ...
        if len(test_df) > 0 and 'target_growth' in test_df.columns:
            ...
            if len(X_test_df) > 0:
                # 예측 수행
                y_test_pred_proba = model.predict_proba(X_test_df)[:, 1]
        ...
        if len(X_test_2024) > 0:
            ...
            if len(actual_target_df) > 0:
                # 2023년 피처로 예측
                y_test_pred_proba = model.predict_proba(X_test_2024)[:, 1]
    ...
    results_dict = {
        'model': ['RandomForest'] * len(metrics_list),
        'metric': metrics_list,
        'groupkfold': [
    ...
    save_performance_report(
        ...
        best_params if best_params else model_params_for_validation, 
        feature_importance_df, script_dir, model_name='RandomForest'
    )
